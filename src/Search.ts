...
class SearchInit implements ISearchInit {
  ...
  createDataset(dataItems: Obj[]): DatasetItemStructure[] {
  ...
  totalScoringModules(haystack: string[], weight: number) {
  ...
  flatSort(itemL: DatasetItemStructure, itemR: DatasetItemStructure) {
  ...
  addSearchModule<T, O>(Module: new(options: O) => T & BaseSearch, options: O) {
